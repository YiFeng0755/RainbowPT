...
public class CpuInfo {
	...
	private static final String LOG_TAG = "Rainbow-" + CpuInfo.class.getSimpleName();
	...
	private long initialTraffic_send;
	private long initialTraffic_receive;
	private long sendTraffic;
	private long receiveTraffic;
	private long lastestTraffic;
	private long lastestTraffic_send;
	private long lastestTraffic_receive;
	...
	private int pid;
	private long initialNetTimeout = 5 * 1000;//初始化网络数据所需要的时间
	 ...
	 *         certain interval
	 */
	public ArrayList<String> getCpuRatioInfo(String totalBatt, String currentBatt, String temperature, String voltage,String fps) {

		DecimalFormat fomart = new DecimalFormat();
	    Long trafficArray[] = null;
		...
		try {
			...
			if (isInitialStatics) {
				 long endTime = System.currentTimeMillis() + initialNetTimeout;
					while(System.currentTimeMillis() < endTime){						
						trafficArray = trafficInfo.getSndAndRcv();
						initialTraffic = trafficArray[2];
						if(initialTraffic != -1){
							initialTraffic_send = trafficArray[0];
							initialTraffic_receive = trafficArray[1];
							break;
						}
						try {
							Thread.sleep(300);
						...
						Log.w(LOG_TAG, "Retry to get traffic information");
					}
				isInitialStatics = false;
			} else {				
				//lastestTraffic = trafficInfo.getTrafficInfoByTrafficStats();
				trafficArray = trafficInfo.getSndAndRcv();
				lastestTraffic = trafficArray[2];
				lastestTraffic_send = trafficArray[0];
				lastestTraffic_receive = trafficArray[1];
				if (initialTraffic == -1)
					traffic = -1;
				else {
					//traffic = (lastestTraffic - initialTraffic + 1023) / 1024;
					sendTraffic = (lastestTraffic_send - initialTraffic_send + 1023) / 1024;
					receiveTraffic = (lastestTraffic_receive - initialTraffic_receive + 1023) / 1024;
					traffic = sendTraffic + receiveTraffic;
					//totalTraffic = ((lastestTraffic - initialTraffic + 1023) / 1024) - previousTraffic ;
					//previousTraffic = totalTraffic;
				}
				...
				if (null != totalCpu2 && totalCpu2.size() > 0) {
					...
					for (int i = 0; i < (totalCpu.size() > totalCpu2.size() ? totalCpu2.size() : totalCpu.size()); i++) {
						...
						//Add by Jack For debug
						/*if(Double.parseDouble(cpuRatio) > 100){
							Log.e(LOG_TAG, "总CPU异常" + Double.parseDouble(cpuRatio));
						}*/
				...
				if (isPositive(processCpuRatio) && isPositive(totalCpuRatio.get(0))) {
					String trafValue, trafValue_send, trafValue_receive;
					
					// whether certain device supports traffic statics or not
					if (traffic == -1) {
						trafValue = NA;
						trafValue_send = NA;
						trafValue_receive = NA;
					} else {
						trafValue = String.valueOf(traffic);						
						trafValue_send = String.valueOf(sendTraffic);
						trafValue_receive = String.valueOf(receiveTraffic);
					}
					/*RainbowPTService.bw.write(mDateTime2 + COMMA + pMemory + COMMA + percent + COMMA + fMemory + COMMA + processCpuRatio + COMMA
							+ totalCpuBuffer.toString() + trafValue + COMMA + totalBatt + COMMA + currentBatt + COMMA + temperature + COMMA + voltage
							+ "\r\n");*/
					RainbowPTService.bw.write(mDateTime2 + COMMA + AppProcessInfo.getTopActivity(context) + COMMA +pMemory + COMMA + percent + COMMA + fMemory + COMMA + processCpuRatio + COMMA + trafValue + COMMA
					+ trafValue_send + COMMA + 	trafValue_receive + COMMA + fps	+ COMMA 
					+ totalCpuBuffer.toString()  + totalBatt + COMMA + currentBatt + COMMA + temperature + COMMA + voltage
					+ "\r\n");
					totalCpu2 = (ArrayList<Long>) totalCpu.clone();
					processCpu2 = processCpu;
					idleCpu2 = (ArrayList<Long>) idleCpu.clone();
